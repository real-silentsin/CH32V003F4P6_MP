 ...
 * Author             : vantr
 * Description        : Библиотека для работы с PWM, 4 канала, TIM1
 *********************************************************************************
...
void SSS_PWM_Init(uint16_t arr, uint16_t psc) {
    ...
    // 1. Включение тактирования TIM1 и портов 
    // Внимание: TIM1 находится на шине APB12
    ...
    GPIO_Init (PWM4_PORT.port, &GPIO_InitStructure);  // CH4
#endif
...
uint16_t TIM1_GetDutyValue(uint8_t percents) {

    return (uint16_t)((percents * arr_private_init) / 100);
}
// -----------------------------------------------------------------------------
// Установка коэффициента заполнения PWM указанного канала
void TIM1_SetDuty_CH(uint8_t channel, uint8_t duty_percents) {

    uint16_t duty_value = TIM1_GetDutyValue(duty_percents);

    switch(channel) {
#ifdef PWM_USE_CH_1
        case 1: TIM_SetCompare1(TIM1, duty_value); break;
#endif
#ifdef PWM_USE_CH_2
        case 2: TIM_SetCompare2(TIM1, duty_value); break;
#endif
#ifdef PWM_USE_CH_3
        case 3: TIM_SetCompare3(TIM1, duty_value); break;
#endif
#ifdef PWM_USE_CH_4
        case 4: TIM_SetCompare4(TIM1, duty_value); break;
#endif
    }
}
// -----------------------------------------------------------------------------
...
void SSS_SetDuty_CH1(uint8_t duty_percents) {
    ...
    TIM1_SetDuty_CH(1, duty_percents);
}
#endif
// -----------------------------------------------------------------------------
#ifdef PWM_USE_CH_2
void SSS_SetDuty_CH2(uint8_t duty_percents) {
    ...
    TIM1_SetDuty_CH(2, duty_percents);
}
#endif
// -----------------------------------------------------------------------------
#ifdef PWM_USE_CH_3
void SSS_SetDuty_CH3(uint8_t duty_percents) {
    ...
    TIM1_SetDuty_CH(3, duty_percents);
}
#endif
// -----------------------------------------------------------------------------
#ifdef PWM_USE_CH_4
void SSS_SetDuty_CH4(uint8_t duty_percents) {
    ...
    TIM1_SetDuty_CH(4, duty_percents);
}
#endif
// -----------------------------------------------------------------------------